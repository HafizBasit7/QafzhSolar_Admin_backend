...
const addShop = async (req, res) => {
  try {
    ...
    if (!name || !phone || !services || !governorate || !addedBy) {
      return res.status(400).json({
        status: "fail",
        message:
          "Required fields are missing: name, phone, services, governorate",
      });
    }

    // Basic phone validation (only check if it's not empty)
    ...
    const newShop = new Shop({
      ...
      notes,
      addedBy,
      isVerified: isVerified || true, // Admin adds = verified by default
