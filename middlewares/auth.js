...
const authToken = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    if (!currentUser || !currentUser.isActive) {
      return next(
        new AppError("The admin account no longer exists or is inactive.", 401)
      );
    }
    ...
    if (!currentUser || !currentUser.isActive) {
      return next(
        new AppError("The user account no longer exists or is inactive.", 401)
      );
    }
  }
...
const isAdmin = (req, res, next) => {
  if (
    !req.user ||
    !["admin", "super_admin", "moderator"].includes(req.user.role)
  ) {
...
const checkPermission = (resource, action) => {
  return (req, res, next) => {
    ...
    if (!req.user.hasPermission || !req.user.hasPermission(resource, action)) {
      return next(
        new AppError(`You don't have permission to ${action} ${resource}`, 403)
      );
    }
